const { isBotAdmin, botJid } = require("./lixon");
...
async function modifyAllGroupRoles(client, msg, action) {
    try {
        ...
        if (!groupJid.endsWith("@g.us"))
            return sendFallback(client, groupJid, "_Use in group!_");

        const chat = await client.groupMetadata(groupJid);
        if (!chat)
            return sendFallback(client, groupJid, "_Metadata error!_");

        if (!isBotAdmin(client, chat.participants))
            return sendFallback(client, groupJid, "_I'm not admin!_");

        let validUsers = [];

        if (action === "promote") {
            validUsers = chat.participants
                .filter(p => !p.admin && p.id !== botJid)
                .map(p => p.id);

        } else if (action === "demote") {
            const allAdmins = chat.participants.filter(p => p.admin);

            validUsers = allAdmins
                .filter(p => p.id !== botJid)
                .map(p => p.id);

            if (validUsers.length === 0)
                return sendFallback(client, groupJid, "_No other admins to demote!_");
        }

        if (validUsers.length === 0)
            return sendFallback(client, groupJid, `_No members to ${action}.`);

        try {
            await client.groupParticipantsUpdate(groupJid, validUsers, action);
            await client.sendMessage(groupJid, {
                text: `✅ ${action.charAt(0).toUpperCase() + action.slice(1)}d ${validUsers.length} member(s).`
            });
        } catch (error) {
            return handleGroupUpdateError(client, groupJid, error);
        }
