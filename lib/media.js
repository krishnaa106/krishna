...
async function extractViewOnceMedia(client, msg, sendToSelf = false) {
    ...
    const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage?.viewOnceMessageV2?.message ||
                   msg.message?.extendedTextMessage?.contextInfo?.quotedMessage?.viewOnceMessageV2Extension?.message;
    
    if (!quoted) return client.sendMessage(jid, { text: "❌ Reply to a view-once media!" });
    
    const mediaType = Object.keys(quoted)[0];
    if (!["imageMessage", "videoMessage", "audioMessage"].includes(mediaType)) 
        return client.sendMessage(jid, { text: "❌ Unsupported media type!" });
    
    const buffer = await downloadMediaMessage({ message: quoted }, "buffer");
    const sendOptions = { [mediaType.replace("Message", "")]: buffer };
    
    if (mediaType === "audioMessage") {
        Object.assign(sendOptions, { mimetype: "audio/ogg; codecs=opus", ptt: true });
    }
