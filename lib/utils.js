...
let stickerSize = {
    ratio: "1:1",      // Aspect ratio (e.g., 1:1, 9:16)
    placement: "center" // Positioning: top, center, bottom
};

/**
 * Get current sticker crop settings
 * @returns {{ ratio: string, placement: string }}
 */
const getStickerSize = () => stickerSize;

/**
 * Set sticker crop settings
 * @param {string} ratio - Aspect ratio (e.g., "1:1", or "0" for free size)
 * @param {string} [placement="_"] - Placement for crop ("top", "center", "bottom")
 */
const setStickerSize = (ratio, placement = "_") => {
    stickerSize = { ratio, placement };
};
...
async function downloadUrl(url) {
    try {
        ...
        const filePath = path.join(TMP_DIR, `media_${Date.now()}.${extension}`);
 ...
 * @param {object} [options.resizeTo] - Optional: { width, height } for resizing
...
async function cropVid(inputPath, outputPath, options = {}) {
    ...
    if (resizeTo?.width && resizeTo?.height) {
...
async function toWebp(inputPath) {
    ...
    const outputPath = path.join(TMP_DIR, `sticker.webp`);
...
async function toggleAtake(client, msg, args) {
...
async function processStickerQueue() {
    ...
    try {
        ...
        if (isWebp) {
            // animated or static webp, copy as-is
        ...
        isProcessing = false;
        setTimeout(processStickerQueue, 300); // tweak delay for spam control
    }
}
...
async function dlStkZip(msg, type) {
    try {
        const quotedMsg = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage?.documentMessage;
        if (!quotedMsg) return null;
        const filePath = path.join(TMP_DIR, "downloaded.zip");
        fs.writeFileSync(filePath, Buffer.concat(await streamToBuffer(await downloadContentFromMessage(quotedMsg, type))));
        return filePath;
    } catch (err) {
