...
async function extractStkZip(zipPath, outputDir) {
    return new Promise((resolve, reject) => {
        fs.createReadStream(zipPath)
            .pipe(unzipper.Parse())
            .on("entry", entry => (entry.type === "File" && (entry.path.endsWith(".webp") || entry.path.endsWith(".webm")))
...
async function processZipStk(client, msg, addExif, shouldStopFn) {
    try {
        const files = fs.readdirSync(TMP_DIR).filter(f => f.endsWith(".webp") || f.endsWith(".webm"));

        if (!files.length) {
            return client.sendMessage(msg.key.remoteJid, { text: "❌ No .webp or .webm files found!" });
        }
        ...
        for (const file of files) {
            ...
            try {
                let finalWebpPath = filePath;

                // Convert .webm to .webp before processing
                if (file.endsWith(".webm")) {
                    finalWebpPath = filePath.replace(".webm", ".webp");
                    await convertWebmToWebp(filePath, finalWebpPath);
                    fs.unlinkSync(filePath); // remove original .webm
                }

                const modifiedPath = await addExif(finalWebpPath);
                ...
                await client.sendMessage(msg.key.remoteJid, { text: `⚠️ Error processing *${file}*.` });
            }
        }
    } catch (err) {
